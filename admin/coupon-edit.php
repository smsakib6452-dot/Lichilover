...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $dup = fetch_one('SELECT id FROM coupons WHERE code = ? AND id <> ?', [$code, $id]);
        if ($dup) {
            $errors['code'] = 'Another coupon already uses this code.';
        } else {
            query(
                'UPDATE coupons SET code = ?, discount_type = ?, discount_value = ?, min_order = ?, max_discount = ?, expires_at = ?, usage_limit = ?, is_active = ? WHERE id = ?',
                [$code, $discountType, $discountValue, $minOrder, $maxDiscount, $expiresAt, $usageLimit, $isActive, $id]
            );
...
$adminTitle = 'Edit Coupon';
require_once __DIR__ . '/includes/header.php';
?>

<form method="post" action="<?= url('admin/coupon-edit.php?id=' . $id) ?>" class="admin-card" style="max-width:640px" data-loading>
    <?= csrf_field() ?>

    <h3 style="margin-bottom:16px">Edit Coupon: <?= e($coupon['code']) ?></h3>

    <div class="form-grid">
        <div class="form-field <?= isset($errors['code']) ? 'has-error' : '' ?>">
            <label>Coupon Code</label>
            <input type="text" name="code" value="<?= e(old('code', $coupon['code'])) ?>" required>
        ...
        <div class="form-field <?= isset($errors['discount_value']) ? 'has-error' : '' ?>">
            ...
            <input type="number" step="0.01" min="0" name="discount_value" value="<?= e($coupon['discount_value']) ?>" required>
            <?php if (isset($errors['discount_value'])): ?><span class="form-error"><?= e($errors['discount_value']) ?></span><?php endif; ?>
        </div>
        <div class="form-field">
            <label>Minimum Order (৳)</label>
            <input type="number" step="0.01" min="0" name="min_order" value="<?= e($coupon['min_order']) ?>">
        </div>
        <div class="form-field">
            <label>Maximum Discount (৳)</label>
            <input type="number" step="0.01" min="0" name="max_discount" value="<?= e($coupon['max_discount']) ?>">
        </div>
        <div class="form-field">
            <label>Expiry Date</label>
            <input type="date" name="expires_at" value="<?= e($coupon['expires_at']) ?>">
        </div>
        <div class="form-field">
            <label>Usage Limit</label>
            <input type="number" min="0" name="usage_limit" value="<?= (int) $coupon['usage_limit'] ?>">
            <span class="form-hint">Used so far: <?= (int) $coupon['used_count'] ?></span>
        </div>
        <div class="form-field">
            <label>&nbsp;</label>
            <label style="display:flex;gap:6px;align-items:center;font-size:14px"><input type="checkbox" name="is_active" value="1" <?= (int) $coupon['is_active'] === 1 ? 'checked' : '' ?>> Active</label>
        </div>
    </div>

    <button type="submit" class="btn btn-primary" style="margin-top:16px">Save Coupon</button>
</form>
